...
if [[ -n "$MI_ID" ]]; then
    ...
    ROLES=$(az role assignment list --assignee "$MI_ID" --all --query "[].roleDefinitionName" -o tsv 2>/dev/null || echo "")
...
AI_NAME=$(az resource list --resource-group "$RESOURCE_GROUP" --resource-type "Microsoft.Insights/components" --query "[?contains(name, 'socradar-entraid')].name" -o tsv 2>/dev/null | head -1)
...
echo ""
echo "[8/8] Workbooks..."
WB_COUNT=$(az resource list --resource-group "$RESOURCE_GROUP" --resource-type "Microsoft.Insights/workbooks" --query "length([?contains(name, 'socradar-entraid') || contains(name, 'cbbcd') || contains(name, '40acb') || contains(name, '3739e') || contains(name, '17f16')])" -o tsv 2>/dev/null || echo "0")

if [[ "$WB_COUNT" -gt 0 ]]; then
    pass "$WB_COUNT SOCRadar workbook(s) deployed"
    az monitor app-insights workbook list --resource-group "$RESOURCE_GROUP" --category "sentinel" \
