...
if [[ -n "$FA_NAME" ]]; then
    ...
    REQUIRED_SETTINGS=("SOCRADAR_API_KEY" "SOCRADAR_COMPANY_ID" "ENTRA_TENANT_ID" "ENTRA_CLIENT_ID" "ENTRA_CLIENT_SECRET" "WORKSPACE_ID" "WORKSPACE_KEY" "STORAGE_ACCOUNT_NAME" "POLLING_SCHEDULE" "INITIAL_LOOKBACK_MINUTES")
    ...
    echo ""
    echo "  Action toggles:"
    for toggle in "ENABLE_REVOKE_SESSION" "ENABLE_ADD_TO_GROUP" "ENABLE_PASSWORD_CHANGE" "ENABLE_DISABLE_ACCOUNT" "ENABLE_CONFIRM_RISKY" "ENABLE_ROPC" "ENABLE_CREATE_INCIDENT" "ENABLE_RESOLVE_ALARM"; do
        VAL=$(echo "$SETTINGS_JSON" | python3 -c "import sys,json; items=json.load(sys.stdin); vals=[i['value'] for i in items if i['name']=='$toggle']; print(vals[0] if vals else '?')" 2>/dev/null || echo "?")
        echo "    $toggle = $VAL"
    done
else
