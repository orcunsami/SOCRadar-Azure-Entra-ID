#!/usr/bin/env python3
"""SOCRadar VIP Data v2 — Edge Case + Date Filter Tests
...
Output: tests/vip/results/edge_cases.json with per-test response summary.
"""
...
def fetch(api_key, company_id, page=1, limit=10, start_date=None, user_agent="SOCRadar-Test/1.0"):
    ...
    url = f"{BASE}/company/{company_id}/vip-protection/v2?page={page}&limit={limit}"
...
def main():
    ...
    results = {"endpoint": "vip-data/v2", "company_id": COMPANY_ID, "tested_at": datetime.now(timezone.utc).isoformat(), "cases": {}}
