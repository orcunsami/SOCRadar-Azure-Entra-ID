#!/usr/bin/env python3
"""SOCRadar PII Exposure v2 API Test

Tests the dark web monitoring PII exposure endpoint.
...
def fetch_pii(page=1, limit=10, start_date=None):
    url = f"{BASE_URL}/company/{COMPANY_ID}/dark-web-monitoring/pii-exposure/v2?page={page}&limit={limit}"
    if start_date:
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=30) as resp:
            ...
            return {
                "endpoint": f"/company/{COMPANY_ID}/dark-web-monitoring/pii-exposure/v2",
                "page": page,
                "limit": limit,
                ...
                "sources": list(set(
                    ...
                    for i in items
                    for s in (i.get("source", []) if isinstance(i.get("source"), list) else [i.get("source", "")])
                ))[:10],
                "error": None,
            }
        ...
        return {"endpoint": "pii-exposure/v2", "status": 0, "error": str(e)}


def main():
    print(f"PII Exposure v2 API Test — company: {COMPANY_ID}")
    print("=" * 60)
    ...
    if r.get("error"):
        ...
        print(f"  Sources: {r['sources'][:5]}")

    # Test 2: Page 2
    print("\n[2/4] Page 2, limit=5...")
    r = fetch_pii(page=2, limit=5)
    results.append(r)
    if r.get("error"):
    ...
    # Test 3: With startDate
    print("\n[3/4] With startDate=2026-03-01...")
    r = fetch_pii(page=1, limit=10, start_date="2026-03-01")
    results.append(r)
    if r.get("error"):
        print(f"  ERROR: {r['error']}")
    else:
    ...
    try:
        req = urllib.request.Request(
            f"{BASE_URL}/company/{COMPANY_ID}/dark-web-monitoring/pii-exposure/v2?page=1&limit=1",
            headers={"API-Key": "invalid_key_test"},
        )
        with urllib.request.urlopen(req, timeout=10) as resp:
            results.append({"status": resp.status, "error": "Expected error"})
    except urllib.error.HTTPError as e:
        results.append({"status": e.code, "error": None})
    ...
    # Save
    output = {"api": "PII Exposure v2", "company_id": COMPANY_ID, "results": results}
    out_path = Path(__file__).parent / "results" / "pii_response.json"
